...
DEFAULT_CONFIG = {
    ...
    "system_prompt": (
        ...
        "code examples. Do NOT end the post with a conclusion section, and never include a "
        "### Conclusion heading."
    ),
    "user_prompt_template": (
        ...
        "- Be well-structured with headings and paragraphs.\n"
        "- Be at least 350 words long.\n"
        "- NOT end with a conclusion section (no 'Conclusion' heading)."
    ),
    "expand_prompt": (
        ...
        "and ensure the final post is comprehensive and well structured. Do not add a conclusion section."
    ),
...
_CONCLUSION_RE = re.compile(
    r"^#{1,6}\s*(conclusion|final\s+thoughts|summary|wrap[- ]?up|in\s+closing)\s*:?\s*$",
    re.IGNORECASE,
)


def strip_conclusion(content):
    """Remove any trailing conclusion/final section from generated content."""
    lines = content.splitlines()
    last_idx = None
    for i, line in enumerate(lines):
        if _CONCLUSION_RE.match(line.strip()):
            last_idx = i
    if last_idx is None:
        return content
...
def generate_blog_post(
    ...
    for attempt in range(1, cfg["max_retries"] + 1):
        ...
        content = strip_conclusion(content)
